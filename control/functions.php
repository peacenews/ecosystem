...
function new_sign_ups() {
    global $page_control;
    global $dbpdo;
    $query = "SELECT w.title as `group`, w.`why`, u.`name`, u.`email`,w.`url`,wu.id as websiteuserid  FROM `users` u,website_users wu,website w WHERE wu.valid=0 AND wu.role_on_Website='Superuser' AND u.ID=wu.user_id and wu.website_id=w.id";
    ...
    while($r = $result->fetch(PDO::FETCH_BOTH)) {
        ...
        <td class="delete"><a href="?delete_user='.$r['websiteuserid'].'" class="confirm">delete</a></td>
        <td class="approve"><a href="?approve_user='.$r['websiteuserid'].'" class="confirm">approve and send email</a></td>
    </tr>';
    }
}
...
function existing_users() {
    ...
    // echo '</pre>';
    if ($page_control->uri=='/manage/users/owners')  $sql="wu.valid=1 AND wu.role_on_Website='Superuser'";
    if ($page_control->uri=='/manage/users/all') $sql="wu.valid=1";
    if ($page_control->uri=='/manage/users') $sql="wu.valid=1";
    if ($page_control->uri=='/manage/users/suspended')  $sql="wu.valid=0";
    if (isset($_POST['usearch']))  $sql="  u.`email` like '%".$_POST['usearch']."%'";
    //$query = "SELECT * FROM `users` WHERE   $sql  order by `email` ";
    //echo $query;
	$query = "SELECT w.title as `group`, w.`why`, u.id, u.`name`, u.`email`,w.`url`,wu.id as websiteuserid ,wu.role_on_Website as  `type` ,wu.email_preferences as `deputies` ,wu.`valid`  FROM `users` u,website_users wu,website w WHERE $sql  AND u.ID=wu.user_id and wu.website_id=w.id order by u.`email`";
	$result = $dbpdo->query($query);
	while($r = $result->fetch(PDO::FETCH_BOTH)) {
        if ($r['valid']=='1')  $drop = '<td class="delete"><a href="?suspend_user='.$r['websiteuserid'].'" class="confirm">suspend</a></td>';
        if ($r['valid']=='0')  $drop = '<td class="delete"><a href="?restate_user='.$r['websiteuserid'].'" class="confirm">re-instate</a></td>';
        if ($r['type']=='Superuser') $r['type']='Owner';
        if(strlen($r['deputies'])>0) $deps=implode(', ',unserialize($r['deputies']));
        echo '<form ><tr>
        <td>'.$r['name'].'</td>
        <td>'.$r['email'].'</td>
        <td>'.$deps.'</td>
        ...
        <td>'.$r['type'].'</td>
        '.$drop.'
        <td class="delete confirm"><a href="?delete_user='.$r['websiteuserid'].'">delete</a></td>
    </tr></form >';
    }
}
...
function contributors() {
    global $page_control;
    global $dbpdo;
    $query = "SELECT * FROM `users` WHERE id in (SELECT user_id FROM `website_users` where website_id='".$_SESSION['public_user']['site_id']."' and valid=1 and role_on_Website='Contributor') order by email  ";
	//echo $query;
    $result = $dbpdo->query($query);
    while($r = $result->fetch(PDO::FETCH_BOTH)) {
...
function participants() {
    global $page_control;
    global $dbpdo;
    $query = "SELECT * FROM `users` WHERE id in (SELECT user_id FROM `website_users` where website_id=".$_SESSION['public_user']['site_id']." and valid=1 and role_on_Website='Participant')";
    $result = $dbpdo->query($query);
    while($r = $result->fetch(PDO::FETCH_BOTH)) {
        echo '<p><a href="/contributors?delete_participant='.$r['id'].'" class="confirm">Delete</a> '.$r['email'].'</p>';
    }
}
...
function  delete_user($user_id) {
    global $dbpdo;
	$get_user = $dbpdo->prepare('select `user_id`, `website_id`, `role_on_Website` from `website_users` where id=:id');
	$get_user->execute(array(':id' => $user_id));
	$get_del_user = $get_user->fetch();
		
	$get_user_count = $dbpdo->prepare('select count(user_id) from `website_users` where user_id=:id');
	$get_user_count->execute(array(':id' => $get_del_user['user_id']));
	$get_del_user_count = $get_user_count->fetchColumn();
	// To delete the user from users table, if the user id is equal to 1.
	if($get_del_user_count == 1){
		$stmt = $dbpdo->prepare('DELETE from users WHERE id in(select `user_id` from `website_users` where id=:id)');
		$stmt->execute(array(':id' => $user_id));
	}
	// To delete the website from website table, if the user is superuser.
	/*$get_website_count = $dbpdo->prepare('select count(website_id) from `website_users` where website_id=:id and role_on_Website=:role');
	$get_website_count->execute(array(':id' => $get_del_user['website_id'], ':role' => $get_del_user['role_on_Website']));
	$get_del_website_count = $get_website_count->fetchColumn();
	
	if($get_del_website_count == 1){
		$website_del = $dbpdo->prepare('DELETE from website WHERE id in(select `website_id` from `website_users` where id=:id)');
		$website_del->execute(array(':id' => $user_id));
		
		$website_users_contrib_part_del = $dbpdo->prepare('DELETE from website_users WHERE website_id=:id');
		$website_users_contrib_part_del->execute(array(':id' => $get_del_user['website_id']));
	}*/
	$stmt = $dbpdo->prepare('DELETE from forwardings  WHERE id in(select `website_id` from `website_users` where id=:id)');
    $stmt->execute(array(':id' => $user_id));
    $stmt = $dbpdo->prepare('DELETE from website_users WHERE id =:id');
    $stmt->execute(array(':id' => $user_id));    
}

function  suspend_user($user_id) {
    global $dbpdo;
    $stmt = $dbpdo->prepare('update  website_users  set `valid`=0 WHERE id = :id');
    $stmt->execute(array(':id' => $user_id));
    $stmt = $dbpdo->prepare('delete from forwardings  WHERE id in(select website_id from website_users where id=:id)');
...
function  restate_user($user_id) {
    global $dbpdo;
    $stmt = $dbpdo->prepare('update  website_users  set `valid`=1 WHERE id = :id');
    $stmt->execute(array(':id' => $user_id));
    $query = "select url, id from website where id in(select website_id from website_users where id=$user_id)";
    $result= $dbpdo->query($query);
    ...
    $query = "INSERT INTO `forwardings` (`destination`, `source`, `id`) VALUES ('vmail', '$disc', $wid)";
    $result = $dbpdo->query($query);
}

function  set_approve_password($user_id) {
    global $dbpdo;
    global $salt;
    $rnd_pass=makeRandomPassword();
    $stmt = $dbpdo->prepare("UPDATE users set `password`= :pass WHERE id in(select user_id from website_users where id= :id)");
    $stmt->execute(array(
        ':id' => $user_id,
...
function send_email($user_id,$rnd_pass,$email) {
    ...
    if ($email=='Check sign up') {
		...
		if($email == 'approval' || $email == 'existinguser_groupapproval'){
			$stmt = $dbpdo->prepare("SELECT name,email from users  WHERE id in(select user_id from `website_users` where id=:id)");
		}else{
...
function  approve_user($user_id) {
    ...
    $stmt->execute(array(':id' => $user_id));
	// echo $rnd_pass;
	$password_update = "SELECT `password` FROM `users` where id in(select `user_id` from `website_users` where id=$user_id)";
	$pwd= $dbpdo->query($password_update);
	$user_pwd = $pwd->fetchColumn();
	if($user_pwd == ''){	
		$rnd_pass=set_approve_password($user_id);
	}else{
		$rnd_pass = '';
		
	}
    $query = "select website_id from website_users where id=$user_id ";
    $result= $dbpdo->query($query);
	//$result->execute();
    $mysql_result = $result->fetchAll(PDO::FETCH_ASSOC);
	//print_r($mysql_result);
	foreach($mysql_result as $my_result){
		$website_data = "select * from website where id=".$my_result['website_id'];
		$website_details= $dbpdo->query($website_data);
		$website_all_details = $website_details->fetch(PDO::FETCH_NUM);
		$wsite_id = $website_all_details[0];
		$url=$website_all_details[14];
		$disc=ltrim($url,'/');
		
		$query = "INSERT INTO `forwardings` (`destination`, `source`, `id`) VALUES ('vmail', '$disc', $wsite_id)";
		$result = $dbpdo->query($query);
		$query = "INSERT INTO .`pages` (`id`, `name`, `url`, `type`, `web_id`, `content`, `pic`, `caption`) VALUES (NULL, '', '', 'Home', $wsite_id, '<h1>Add your Heading for your introduction here</h1>
		...
		$result = $dbpdo->query($query);
		$query = "INSERT INTO .`pages` (`id`, `name`, `url`, `type`, `web_id`, `content`, `pic`, `caption`) VALUES (NULL, 'News', 'news', 'News', $wsite_id, '', '', ''); ";
		$result = $dbpdo->query($query);
	}
	return $rnd_pass;
}
...
function add_page($page_name, $type, $page_url) {
    global $dbpdo;
    if ($type=='') $type='Page';
	$sid = $dbpdo->prepare("SELECT id FROM `website` where title ='".$_SESSION['public_user']['site_title']."'");
	$sid->execute();
	$sdata = $sid->fetch(PDO::FETCH_ASSOC);	
    $stmt = $dbpdo->prepare("INSERT INTO .`pages` (`id`, `name`, `url`, `type`, `web_id`, `content`, `pic`, `caption`) VALUES (NULL, :page, :url, :type, ".$sdata['id'].",
