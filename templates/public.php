...
<body class="website <?php echo $body_class ?>">
    ...
    <div class="logged-in-menu website">
        <div class="row">
            ...
            <div class="large-4 small-8 medium-2 columns"><p class="regular-zylum"><?php //echo $_SESSION['public_user']['site_title'];
			$stmt = $dbpdo->prepare("SELECT title,url FROM `website` where id in (SELECT website_id FROM `website_users` where user_id='".$_SESSION['public_user']['user_id']."' and valid=1 )");
			$stmt->execute();
			$result = $stmt->fetchAll();			
			foreach($result as $key=>$website){
				echo '<a href=?url='.$website['url'] .'>'. $website['title'].'</a> | ';				
			}
